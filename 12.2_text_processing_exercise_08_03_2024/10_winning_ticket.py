def check_ticket(ticket):
    ...
    for current_winning_symbols in winning_symbols:
        for uninterrupted_match_length in range(10, 5, -1):
            winning_symbol_repetition = current_winning_symbols * uninterrupted_match_length
            if winning_symbol_repetition in left_part and winning_symbol_repetition in right_part:
                if uninterrupted_match_length == 10:
                    return f'ticket "{ticket}" - {uninterrupted_match_length}{current_winning_symbols} Jackpot!'
                return f'ticket "{ticket}" - {uninterrupted_match_length}{current_winning_symbols}'
